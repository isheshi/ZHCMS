...
(function($,undefined){
	$.fn.uploadsFile = function(options,param){
		...
		if (typeof options == 'string') {
			...
			if($.isFunction(fn)){
				...
				throw ("uploadsFile - No such method: " + options);
			}
		}
		...
		return this.each(function(){
			...
			var defaults = {
					...
					UpFileUrl        : "/FilesAdmin/js/uploadsFile/",  						// 上传插件目录
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';//?fileUrl='+para.fileUrl+'&fileType='+para.fileType+'
					//html += '	<input type="hidden" name="fileUrl" value="isClass" /><input type="hidden" name="fileType" value="pic" />';
					...
					// 创建不带有拖动的html
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="application/x-www-form-urlencoded">';
					html += '	<input type="hidden" name="fileUrl" value="isClass" /><input type="hidden" name="type" value="pic" />';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main single_main">';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。<strong>浏览器必须支持html5才能使用该上传插件</strong></div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="file[]" '+multiple+'>';
					html += '					<div class="webuploader_close">关闭上传</div>';
				    ...
				    html += '							<img class="upload_image" src="'+para.UpFileUrl+'control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("选中"+num+"张文件，共"+size+"。<strong>浏览器必须支持html5才能使用该上传插件</strong>");
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-3;
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = para.UpFileUrl+"control/images/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#tulist").append('<dl id="tulist_'+file.index+'"><dt><span class="zt2">未保存</span><img src="'+$("#uploadList_" + file.index+" #uploadImage_"+file.index).attr("src")+'" width="100" height="80" /></dt></dl>');
						//alert($("#uploadList_" + file.index+" #uploadImage_"+file.index).attr("src"));
			...
			this.addEvent = function(){
				...
				if($(".webuploader_close").length > 0){
					$(".webuploader_close").bind("click", function(e){
						$("#preview .file_del").click();
						$('.uploadFile').fadeOut(200,function(){ $("#uploadForm").remove(); });
		            });
				}
